...
public class CacheWriterInterceptor extends JmxStatsCommandInterceptor {
    ...
    @Inject
    protected void init(PersistenceManager pm, InternalEntryFactory entryFactory, TransactionManager transactionManager, @ComponentName(CACHE_MARSHALLER) StreamingMarshaller marshaller) {
    ...
    protected void commitCommand(TxInvocationContext ctx) throws Throwable {
        if (!ctx.getCacheTransaction().getAllModifications().isEmpty()) {
            ...
            if (getLog().isTraceEnabled()) {
                getLog().tracef("Calling loader.commit() for transaction %s", tx);
            }
    ...
    protected final void store(TxInvocationContext ctx) throws Throwable {
        ...
        if (getLog().isTraceEnabled()) {
            getLog().tracef("Cache loader modification list: %s", modifications);
        }
    ...
    void storeEntry(InvocationContext ctx, Object key, FlagAffectedCommand command) {
        InternalCacheValue sv = getStoredValue(key, ctx);
        persistenceManager.writeToAllStores(new MarshalledEntryImpl(key, sv.getValue(), internalMetadata(sv), marshaller), skipSharedStores(ctx, key, command));
        if (getLog().isTraceEnabled()) {
            getLog().tracef("Stored entry %s under key %s", sv, key);
        }
    }
